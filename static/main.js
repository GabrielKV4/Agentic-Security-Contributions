var app = new Vue({
    ...
    methods: {
        ...
        loadConfigs: async function () {
            ...
            const cfgResponse = await fetch(`${SELF_URL}/config`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        getFailureRateScore(failureRate) {
            const value = Number(failureRate);
            if(!isFinite(value)) return 0;
            return _getFailureRateScore(value);
        },
        getFailureRateColor(failureRate) {
            const value = Number(failureRate);
            if(!isFinite(value)) return 'gray'
            return _getFailureRateColor(value);
        },
        toggleParams() {
        ...
        newEvent: function (event) {
            ...
            this.latency = event.latency ? event.latency.toFixed(3) : this.latency;
            ...
            if (this.mainTable.length < 1) {
                const newRow = {
                    ...event,
                    controllerFailureRate: event.controllerFailureRate ?? 0,
                    failureRate: event.failureRate ?? 0,
                    last: true,
                };
                this.mainTable.push(newRow);
                return;
            }
            ...
            if (last.module === event.module) {
                const updated = {
                ...last,
                tokens: event.tokens ?? last.tokens,
                cost: event.cost ?? last.cost,
                progress: event.progress ?? last.progress,
                failureRate: event.failureRate ?? last.failureRate,
                controllerFailureRate:
                    event.controllerFailureRate !== undefined
                        ? event.controllerFailureRate
                        : last.controllerFailureRate ?? 0,
            };

            this.mainTable.splice(this.mainTable.length - 1, 1, updated);
                ...
                this.mainTable = [...this.mainTable, newRow];
                this.newRow();
            }
