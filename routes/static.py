...
@router.get("/config-loader.js")
async def config_loader_js() -> FileResponse:
    """Serve the config loader JavaScript file."""
    return get_static_file(STATIC_DIR / "config-loader.js")
...
@router.get("/agentic_security.toml")
async def server_config_toml():
    return FileResponse(Path("agentic_security.toml"))

@router.get("/config")
async def config():
    import toml
    data = toml.load("agentic_security.toml")
    return {
        "controllerEnabled": data.get("controller_llm", {}).get("enabled", False)
    }
