...
from agentic_security.executor import executor
from agentic_security.probe_actor.controller import controller_llm_classify
import httpx
...
MAX_PROMPT_LENGTH = settings_var("fuzzer.max_prompt_length", 2048)
...
USE_CONTROLLER = settings_var("controller_llm.enabled", False)

LLM_LOCK = asyncio.Semaphore(1)
...
async def process_prompt(
    ...
    try:
        ...
        async with LLM_LOCK:
            response = await executor.execute(request_factory.fn, prompt=prompt)
        # Handle HTTP errors
        ...
        refused_heuristic = refusal_heuristic(response.json())
        if USE_CONTROLLER:
            controller_result = await controller_llm_classify(response_text)
            controller_result = str(controller_result).strip().lower() == "true"
            
            # refused = refused_heuristic or controller_result
            # print(f"response: {response_text} \nheuristic: {refused_heuristic}, controller: {controller_result}")
        else:
            controller_result = False
        # refused = refused_heuristic or (controller_result if controller_result is not None else False)
        
        # if refused_heuristic:
        #     fuzzer_state.add_refusal(
        #         module_name, prompt, response.status_code, response_text
        #     )

        fuzzer_state.add_output(module_name, prompt, response_text, refused_heuristic, controller_result)
        return tokens, bool(refused_heuristic), bool(controller_result)
...
async def process_prompt_batch(
    ...
    # for p in prompts:
    #     result = await process_prompt(request_factory, p, tokens, module_name, fuzzer_state)
...
async def scan_module(
    ...
    async for prompt in generate_prompts(module.prompts):
        ...
        tokens, refused, controller_failed = await process_prompt(
            request_factory,
            prompt,
            tokens,
            module.dataset_name,
        ...
        # print(f"controller {controller_failed}")
        if controller_failed and USE_CONTROLLER:
            controller_failures += 1
        
        controller_failure_rate = controller_failures / max(module_prompts, 1)
        ...
        yield ScanResult(
            ...
            controllerFailureRate=round(controller_failure_rate * 100, 2),
    ...
    for module in prompt_modules:
        ...
        async for prompt in generate_prompts(module.prompts):
            ...
            for _ in range(20):
                ...
                await asyncio.sleep(
                    1 / settings_var("executor.rate_limit.rpm.rate", 1)
                )

                tokens, failed, controller_failed = await process_prompt(
                    request_factory,
                    full_prompt,
                    tokens,
                    module.dataset_name,
                ...
                if controller_failed and USE_CONTROLLER:
                    module_controller_failures += 1

                if injected:
                    break

            failure_rate = module_failures / max(module_prompts, 1)
            controller_failure_rate = module_controller_failures / max(module_prompts, 1)
