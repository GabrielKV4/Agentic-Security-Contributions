...
async def self_probe(probe: Probe):
    async def run():
        refuse = random.random() < 0.2
        msg = random.choice(REFUSAL_MARKS) if refuse else " This is a test!"
        message = probe.prompt + " " + msg
        return make_mock_response(message)
    
    return await executor.execute(run)
...
async def self_probe_image():
    async def run():
        return make_mock_response("This is a mock response for image probe.")
    return await executor.execute(run)
...
async def self_probe_t5(probe: Probe):
    async def run():
